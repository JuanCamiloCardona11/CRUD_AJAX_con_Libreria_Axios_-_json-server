...
const obtenerPersonas = async () => {
    try{
        let res = await axios.get("http://localhost:5555/personas"),
            json = await res.data;
            console.log(json);
    ...
    if(e.target === $form){
        ...
        if(!e.target.id.value){
            ...
            try{
                ...
                let res = await axios("http://localhost:5555/personas", options),
                json = await res.data;
                location.reload();
            }catch(error){
                let mensaje = error.statusText || "Ha ocurrido un error";
            ...
            try{
                ...
                let res = await axios(`http://localhost:5555/personas/${e.target.id.value}`, options),
                json = await res.data;
                location.reload();
            } catch(error) {
                let mensaje = error.statusText || "Ha ocurrido un error";
    ...
    if(e.target.matches(".eliminar")){
        ...
        if(confirEliminacion){
            ...
            try{
                ...
                let res = await axios(`http://localhost:5555/personas/${e.target.dataset.id}`, options),
                json = await res.data;
                location.reload();
            } catch(error) {
                let mensaje = error.statusText || "Ha ocurrido un error";
